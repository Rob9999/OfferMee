...
def edit(
    ...
    edited_record = container.get_value(record_path)
    ...
    if edited_record and wants_to_store:
        log_info(__name__, f"Processing edited record data ...")
        # container.dump(path=Config.get_instance().get_user_temp_dir())
        # Ask to store changes
        if st.button(_T("Save")):
            log_info(__name__, f"Updating record data ...")
            facade.update(edited_record, operator)
            st.success(_T("Record Data Upadated!"))
            log_info(__name__, f"Record Data Updated!")
