...
def main():
    ...
    try:
        ...
        offers_to_check = (
            ...
            .filter(EditedProjectModel.offer_status == OfferStatus.SENT)
            .all()
        )
        ...
        for offer in offers_to_check:
            ...
            if offer.followup_count >= MAX_FOLLOWUPS:
                continue

            # Time since the last follow-up or initial sending
            last_time = offer.last_followup_date or offer.sent_date
            ...
            body = (f"Dear {offer.contact_person or 'Interested Party'},\n\n"
            ...
            email_utils.send_email(
                ...
                # Update database
                offer.last_followup_date = now
                offer.followup_count += 1
                session.commit()
